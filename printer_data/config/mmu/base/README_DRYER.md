...
A non-blocking filament drying system for Klipper using the `heater_box`.
...
1. Ensure the `mmu_dryer.py` file is in your `extras` directory (e.g., `Happy-Hare/extras/`)
2. Add the following to your `printer.cfg` or include the `mmu_dryer.cfg` file:
   ```
   [include mmu/base/mmu_dryer.cfg]
   ```
3. **Configure the heater name** in `mmu_dryer.cfg` if you're not using `heater_box`:
   ```ini
   [mmu_dryer]
...
```ini
[mmu_dryer]
heater: heater_box          # For Qidi Q2 Box MMU setups
# heater: heater_mmu        # Alternative MMU heater name
...
- The dryer uses your existing `heater_box` heater configuration
- Ensure `heater_box` is properly configured with appropriate safety limits
...
**Error: "Unable to find heater 'heater_box'"**
- Ensure your heater is properly defined in your config
...
- Example: Change `heater: heater_box` to `heater: heater_mmu`
