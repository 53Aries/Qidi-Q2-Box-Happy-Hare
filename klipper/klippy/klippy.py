...
try:
    ...
    pyudev = None
    HAVE_PYUDEV = False
    logging.warning("pyudev not available; USB device monitoring disabled")
import shutil, configparser

# List of "extras" modules to explicitly disable loading for.
# Add any module name (filename without .py or package dir name)
# here to prevent it being imported by import_test or loaded
# via Printer.load_object even if files remain in the tree.
# (disabled-extras feature removed) 
