# AHT10/AHT20/AHT21 I2c-based humidity/temperature sensor support
# Modified for faster sampling suitable for heater control
#
# Copyright (C) 2023 Scott Mudge <mail@scottmudge.com>
# Modified for faster heater control
#
...
class AHT10:
    def __init__(self, config):
        ...
        # Modified: Allow faster reporting for heater control (min 1 second)
        self.report_time = config.getfloat('aht10_report_time', 5., minval=1.)
        self.temp = self.min_temp = self.max_temp = self.humidity = 0.
        self.sample_timer = self.reactor.register_timer(self._sample_aht10)
        self.printer.add_object("aht10 " + self.name, self)
        if config.get('sensor_type', None) == 'AHT2X':
            # Also register as aht2x for compatibility
            self.printer.add_object("aht2x " + self.name, self)
    ...
    def _make_measurement(self):
        ...
        try:
            while is_busy:
                ...
                # Wait 85ms after first read (minimum 75ms, typical 85ms)
                self.reactor.pause(self.reactor.monotonic() + .085)
        ...
        self.temp = ((temp * 200.0) / 1048576.0) - 50.0
        hum = ((data[1] << 16) | (data[2] << 8) | data[3]) >> 4
        self.humidity = int(hum * 100.0 / 1048576.0)
...
def load_config(config):
    ...
    pheater.add_sensor_factory("AHT2X", AHT10)
    pheater.add_sensor_factory("AHT20_F", AHT10)  # Alias for QIDI compatibility
